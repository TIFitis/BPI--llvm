...
static cl::opt<bool> PrintBranchProb("print-bpi",
		cl::init(false),
		cl::Hidden,
		cl::desc("Print the branch probability info."));

cl::opt<std::string> PrintBranchProbFuncName("print-bpi-func-name",
		cl::Hidden,
...
INITIALIZE_PASS_BEGIN(BranchProbabilityInfoWrapperPass, "branch-prob",
	...
	INITIALIZE_PASS_END(BranchProbabilityInfoWrapperPass,
			"branch-prob",
...
void BranchProbabilityInfo::updatePostDominatedByUnreachable(const BasicBlock *BB) {
	...
	for (auto *I : successors(BB))
		...
		if (!PostDominatedByUnreachable.count(I)) return;

	PostDominatedByUnreachable.insert(BB);
}

/// Add \p BB to PostDominatedByColdCall set if applicable.
void BranchProbabilityInfo::updatePostDominatedByColdCall(const BasicBlock *BB) {
...
bool BranchProbabilityInfo::calcUnreachableHeuristics(const BasicBlock *BB) {
	...
	if (UnreachableEdges.empty()) return false;
	...
	auto ReachableProb = (BranchProbability::getOne()
			- UR_TAKEN_PROB * UnreachableEdges.size()) / ReachableEdges.size();
...
bool BranchProbabilityInfo::calcEquality(const BasicBlock *BB) {
	const Instruction *TI = BB->getTerminator();
	assert(TI->getNumSuccessors() > 1 && "expected more than one successor!");
	if (!(isa<BranchInst>(TI))) return false;
	...
	if (!isa<LoadInst>(LHS) || !isa<LoadInst>(RHS)) return false;

	const Value *realLHS = dyn_cast<LoadInst>(LHS)->getOperand(0);
	const Value *realRHS = dyn_cast<LoadInst>(RHS)->getOperand(0);

	if (realLHS != realRHS) return false;

	const Instruction *FI, *LI;
	for (const Instruction &I : BB->instructionsWithoutDebug()) {
		if (&I == cast<Instruction>(LHS)) {
			FI = &I;
			LI = cast<Instruction>(RHS);
			break;
		}
		if (&I == cast<Instruction>(RHS)) {
			FI = &I;
			LI = cast<Instruction>(LHS);
			break;
		}
	}

	FI = FI->getNextNonDebugInstruction();
	while (FI != LI && FI) {
		if (FI->mayWriteToMemory()) {
			for (auto *V : FI->operand_values()) {
				if (V == realLHS) return false;
			}
		}
		FI = FI->getNextNonDebugInstruction();
	}

	setEdgeProbability(BB, 0, BranchProbability(1, 1));
	setEdgeProbability(BB, 1, BranchProbability(0, 1));
	return true;
}
...
bool BranchProbabilityInfo::calcMetadataWeights(const BasicBlock *BB) {
	...
	assert(WeightSum <= UINT32_MAX && "Expected weights to scale down to 32 bits");
...
bool BranchProbabilityInfo::calcColdCallHeuristics(const BasicBlock *BB) {
	...
	if (ColdEdges.empty()) return false;
	...
	auto ColdProb = BranchProbability::getBranchProbability(CC_TAKEN_WEIGHT,
			(CC_TAKEN_WEIGHT + CC_NONTAKEN_WEIGHT)
					* uint64_t(ColdEdges.size()));
	auto NormalProb =
			BranchProbability::getBranchProbability(CC_NONTAKEN_WEIGHT,
					(CC_TAKEN_WEIGHT + CC_NONTAKEN_WEIGHT)
							* uint64_t(NormalEdges.size()));
...
bool BranchProbabilityInfo::calcPointerHeuristics(const BasicBlock *BB,
	...
	if (!CI || !CI->isEquality()) return false;

	Value *LHS = CI->getOperand(0);
	auto new_ph_taken_weight = PH_TAKEN_WEIGHT;
	auto new_ph_not_taken_weight = PH_NONTAKEN_WEIGHT;
	if (AA)
		if (const auto &AAR = AA->alias(CI->getOperand(0), CI->getOperand(1))) {
			if (AAR == AliasResult::MustAlias) {
				new_ph_taken_weight = 0;
				new_ph_not_taken_weight = 1;
			}
			else if (AAR == AliasResult::NoAlias) {
				new_ph_taken_weight = 1;
				new_ph_not_taken_weight = 0;
			}
		}

	if (!LHS->getType()->isPointerTy()) return false;
	...
	if (!isProb) std::swap(TakenIdx, NonTakenIdx);

	BranchProbability TakenProb(new_ph_taken_weight,
			new_ph_taken_weight + new_ph_not_taken_weight);
...
static int getSCCNum(const BasicBlock *BB,
	...
	if (SccIt == SccI.SccNums.end()) return -1;
	return SccIt->second;
}
...
static void computeUnlikelySuccessors(const BasicBlock *BB,
	...
	if (!CI || !isa<Instruction>(CI->getOperand(0))
			|| !isa<Constant>(CI->getOperand(1))) return;
	...
	while (!CmpPHI && CmpLHS && isa<BinaryOperator>(CmpLHS)
		...
		if (!L->contains(CmpLHS)) return;
		InstChain.push_back(cast<BinaryOperator>(CmpLHS));
		CmpLHS = dyn_cast<Instruction>(CmpLHS->getOperand(0));
		if (CmpLHS) CmpPHI = dyn_cast<PHINode>(CmpLHS);
	}
	if (!CmpPHI || !L->contains(CmpPHI)) return;
	...
	while (!WorkList.empty()) {
		...
		for (BasicBlock *B : P->blocks()) {
			...
			if (PHINode *PN = dyn_cast<PHINode>(V)) {
				if (VisitedInsts.insert(PN).second) WorkList.push_back(PN);
				continue;
			}
			...
			if (!CmpLHSConst
					|| std::find(succ_begin(BB), succ_end(BB), B)
							== succ_end(BB)) continue;
			// First collapse InstChain
			for (Instruction *I : llvm::reverse(InstChain)) {
				CmpLHSConst = ConstantExpr::get(I->getOpcode(),
						CmpLHSConst,
						cast<Constant>(I->getOperand(1)),
						true);
				if (!CmpLHSConst) break;
			}
			if (!CmpLHSConst) continue;
...
bool BranchProbabilityInfo::calcLoopBranchHeuristics(const BasicBlock *BB,
	...
	if (!L) {
		...
		if (SccNum < 0) return false;
	}
	unsigned int tc = 0;
	if (SE) {
		tc = SE->getSmallConstantTripCount(L);
	}
	if (tc) tc--;

	SmallPtrSet<const BasicBlock*, 8> UnlikelyBlocks;
	if (L) computeUnlikelySuccessors(BB, L, UnlikelyBlocks);
	...
	unsigned new_taken_weight = LBH_TAKEN_WEIGHT;
	unsigned new_not_taken_weight = LBH_NONTAKEN_WEIGHT;
	unsigned new_unikely_weight = LBH_UNLIKELY_WEIGHT;
	if (tc) {
		new_taken_weight = tc;
		new_not_taken_weight = 1;
		new_unikely_weight = new_taken_weight / 2;
	}
	unsigned Denom = (BackEdges.empty() ? 0 : new_taken_weight)
			+ (InEdges.empty() ? 0 : new_taken_weight)
			+ (UnlikelyEdges.empty() ? 0 : new_unikely_weight)
			+ (ExitingEdges.empty() ? 0 : new_not_taken_weight);

	if (uint32_t numBackEdges = BackEdges.size()) {
		BranchProbability TakenProb = BranchProbability(new_taken_weight,
				Denom);
	...
	if (uint32_t numExitingEdges = ExitingEdges.size()) {
		BranchProbability NotTakenProb = BranchProbability(new_not_taken_weight,
				Denom);
	...
	if (uint32_t numUnlikelyEdges = UnlikelyEdges.size()) {
		BranchProbability UnlikelyProb = BranchProbability(new_unikely_weight,
				Denom);
	...
	if (Instruction *LHS = dyn_cast<Instruction>(CI->getOperand(0)))
		if (LHS->getOpcode() == Instruction::And)
			if (ConstantInt *AndRHS = dyn_cast<ConstantInt>(LHS->getOperand(1)))
				if (AndRHS->getValue().isPowerOf2()) return false;
...
// Hot probability is at least 4/5 = 80%
	if (MaxProb > BranchProbability(4, 5)) return MaxSucc;
...
BranchProbability BranchProbabilityInfo::getEdgeProbability(const BasicBlock *Src,
	...
	if (I != Probs.end()) return I->second;
...
void BranchProbabilityInfo::setEdgeProbability(const BasicBlock *Src,
	...
	LLVM_DEBUG(dbgs() << "set edge "
			<< Src->getName()
			<< " -> "
			<< IndexInSuccessors
			<< " successor probability to "
			<< Prob
			<< "\n");
}
...
void BranchProbabilityInfo::eraseBlock(const BasicBlock *BB) {
	for (auto I = Probs.begin(), E = Probs.end(); I != E; ++I) {
		auto Key = I->first;
		if (Key.first == BB) Probs.erase(Key);
	}
}
...
void BranchProbabilityInfo::calculate(const Function &F,
		...
		ScalarEvolution *SE,
		AAResults *AA) {
	LLVM_DEBUG(dbgs() << "---- Branch Probability Info : "
			<< F.getName()
			<< " ----\n\n");
...
// the successors of a block iteratively.
	for (auto BB : post_order(&F.getEntryBlock())) {
		LLVM_DEBUG(dbgs() << "Computing probabilities for "
				<< BB->getName()
				<< "\n");
		updatePostDominatedByUnreachable(BB);
		updatePostDominatedByColdCall(BB);
		// If there is no at least two successors, no sense to set probability.
		if (BB->getTerminator()->getNumSuccessors() < 2) continue;
		if (calcEquality(BB)) continue;
		if (calcMetadataWeights(BB)) continue;
		if (calcInvokeHeuristics(BB)) continue;
		if (calcUnreachableHeuristics(BB)) continue;
		if (calcColdCallHeuristics(BB)) continue;
		if (calcLoopBranchHeuristics(BB, LI, SccI, SE)) continue;
		if (calcPointerHeuristics(BB, AA)) continue;
		if (calcZeroHeuristics(BB, TLI)) continue;
		if (calcFloatingPointHeuristics(BB)) continue;
	}
...
void BranchProbabilityInfoWrapperPass::getAnalysisUsage(AnalysisUsage &AU) const {
...
BranchProbabilityInfo BranchProbabilityAnalysis::run(Function &F,
...
PreservedAnalyses BranchProbabilityPrinterPass::run(Function &F,
		FunctionAnalysisManager &AM) {
